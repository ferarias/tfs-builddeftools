...
namespace TfsBuildRelationships
{
    ...
    ///     for a <see>
    ///         <cref>TfsBuildRelationships.DependencyGraph</cref>
    ///     </see>
    /// </summary>
    public static class CircularReferencesHelper
    {
        /// <summary>
        ...
        /// <returns></returns>
        public static List<List<T>> FindCircularReferences<T>(DependencyGraph<T> dependencies, List<T> startNodes, List<T> endNodes) where T : IComparable
        {
        ...
        private static void FindCircularReferencesForNode<T>(T node, DependencyGraph<T> dependencies, ref Stack<T> processing, ref HashSet<T> processed,
            ref List<List<T>> circularReferences)
        {
